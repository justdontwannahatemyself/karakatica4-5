...
contract RpcGame{
    ...
    modifier onlyUnique(int roomID) {
        ...
        require(
            (room.player1.playerAddress != tx.origin) && (room.player2.playerAddress != tx.origin), 
    ...
    modifier onlyByPlayers(int roomID) {
        ...
        require((tx.origin == room.player1.playerAddress) || (tx.origin == room.player2.playerAddress), "Not your game");
        _;
    }
    ...
    function joinRoom(int roomID, bytes32 turn) public onlyUnique(roomID) onlyFreeRoom(roomID) {
        ...
        player.playerAddress = tx.origin;
    ...
    function decodeTurn(int roomID, bytes32 salt) public onlyByPlayers(roomID) onlyAfterTurns(roomID) {
        ...
        if(room.player1.playerAddress == tx.origin){
            player = room.player1;
        } 
        else {
            player = room.player2;
